...
\copy (SELECT DISTINCT project_id, repo_id, dep_id, project, repo_name, dep_name FROM (SELECT * FROM p_deps UNION SELECT * FROM repo_deps) AS all_deps ORDER BY project_id, repo_id, dep_id) to './filtered_deps.csv' csv header
